...
sub record_image_retrieved_from {
    ...
    my $file = $image_dir . "/etc/rsyphon/IMAGE_RETRIEVED_FROM";
...
sub record_image_retrieval_time {
    ...
    my $file = $image_dir . "/etc/rsyphon/IMAGE_RETRIEVAL_TIME";
...
sub _write_out_umount_commands {
    ...
    #
    $xml_config = XMLin($file, keyattr => { isys => "+devstyle"} );
    if( defined($xml_config->{isys}->{devstyle}) 
        && (    ("$xml_config->{isys}->{devstyle}" eq "udev" )
             or ("$xml_config->{isys}->{devstyle}" eq "devfs") )
      ) {
        $fs_by_mp{'/dev'} = "/dev";
    }
...
#    print $out "    sed -i \"s:DEFAULTBOOT = rsyphon:DEFAULTBOOT = \$IMAGENAME:g\" $sc_conf_file\n";
#    print $out "    sed -i \"s:LABEL = rsyphon:LABEL = \$IMAGENAME:g\" $sc_conf_file\n";
#    print $out "fi\n";
#}
    ...
    while (<$TEMPLATE>) {
        SWITCH: {
	        if (/^\s*${delim}VERSION_INFO${delim}\s*$/) {
	            print $MASTER_SCRIPT "# This master autoinstall script was created with rsyphon v${VERSION}\n";
	            last SWITCH;
	        }
            ...
            #if (/^\s*${delim}ISYS_DEVSTYLE${delim}\s*$/) {
            #_write_isys_devstyle_entry($MASTER_SCRIPT, $auto_install_script_conf);
            #    #    last SWITCH;
            #}
...
#sub _write_isys_devstyle_entry {
#
#    my ($script, $file) = @_;
#
#    my $xml_config = XMLin($file, keyattr => { isys => "+devstyle"} );
#
#    if( defined($xml_config->{isys}->{devstyle}) 
#        && (    ("$xml_config->{isys}->{devstyle}" eq "devfs")
#             or ("$xml_config->{isys}->{devstyle}" eq "udev" ) )
#      ) {
#
