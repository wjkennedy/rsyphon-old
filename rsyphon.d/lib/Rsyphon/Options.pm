...
return << "EOF";
Usage: rs_pushupdate [OPTION]... --client HOSTNAME   --server HOSTNAME --image IMAGENAME --updateclient-options "[OPTION]..."
  or   rs_pushupdate [OPTION]... --clients-file FILE --server HOSTNAME --updateclient-options "[OPTION]..."
...
return << "EOF";
Usage: rs_getimage [OPTION]...  --golden-client HOSTNAME --image IMAGENAME
...
return << "EOF";
Usage: rs_updateclient [OPTION]... --server HOSTNAME
...
return << "EOF";
 ...
 --updateclient-options "[OPTION]..."
    Pass all options within \"quotes\" to rs_updateclient directly.  Note that
    rs_updateclient\'s --image option need not be specified as it will be
    overridden by rs_pushupdate\'s --image option, or by settings in the file
    specified with --clients-file.
    
 --range N-N
    Number range used to create a series of host names based on the --client
    option.  For example, "--client www --range 1-3" will cause rs_pushupdate
    to use www1, www2, and www3 as host names.  If no --range is given with 
    --client, then rs_pushupdate assumes that only one client is to be updated.
 ...
 --continue-install
    ...
    See "perldoc rs_pushinstall" for more details.

 --ssh-user USERNAME
    Username for ssh connection _to_ the client.  Seperate from 
    rs_updateclient\'s --ssh-user option.
 ...
 --log "STRING"
    ...
    is in addition to the --log option that gets passed to rs_updateclient.


Options for --updateclient-options:
    (The following options will be passed on to the rs_updateclient command.)
...
return << "EOF";
 ...
 --image IMAGENAME
    ...
    used the image defined on the image server by rs_clusterconfig(8).
...
Tip: Use \"rs_lsimage --server HOSTNAME\" to get a list of available images.
...
The following options affect the autoinstall client after autoinstalling:
 ...
 --ip-assignment METHOD
    ...
    DHCP
        ...
        IP address each time, see "man rs_mkdhcpstatic".
...
return << "EOF";
Usage: rs_mkclientnetboot --netboot   --clients "HOST1 HOST2 ..."
  or   rs_mkclientnetboot --localboot --clients "HOST1 HOST2 ..."
