...
# example: rsyphon::Common->which($file,$ENV{PATH}) || croak "$file not found in your path.";
sub which {
...
sub write_auto_install_script_conf_header {
    ...
    open (DISK_FILE, ">$file") or die ("FATAL: Couldn't open $file for writing!"); 
        ...
        print DISK_FILE qq(  rsyphon server to create an autoinstall script for cloning this\n);
    ...
    open(FH_OUT,">>$auto_install_script") or croak("Couldn't open $auto_install_script for appending.");
        ...
        while(<FH_IN>) {
            ...
            if ((/^[[:space:]]*#/) or (/^[[:space:]]*$/)) {
                ...
                # rsyphon::Server->_write_out_new_fstab_file routine. -BEF-
                if ($_ eq "") 
                    { $_ = " "; }
...
EOF
                ...
                if ($mounted) {
                    ...
                    if (($fs eq "vfat") or ($fs eq "msdos")) {
                        ...
                        #
                        if( ! rsyphon::Common->which("minfo",$ENV{PATH}) ) {
                            print << "EOF";
