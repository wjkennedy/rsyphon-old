...
# rs_initrdtools
#
# A dialog/bash utility to manipulate a Rsyphon initrd image
...
trap 'umount $TMPDIR/mount; $VIEWER $LOG; echo "INT caught - exiting rs_initrdtools"; rm *.$$; exit $USER_INTERRUPT' TERM INT
...
GO_STATEMENT="Choose a Rsyphon initrd to modify."
	...
	0)
        ...
        if [ $? != "0" ]
            then
                ...
                $DIALOG --msgbox "Hmm..  That doesn't look like a 2.6 kernel module.\n Make sure the modules you select end with .ko\nAlso, be aware that what you select needs to match the kernel in the RAM disk.  FYI, you're running $KERN_VER." 0 0
...
ACTION=`$DIALOG --stdout --radiolist "Select action to perform on $INITRDUP" 15 80 30 \
...
$DIALOG --yesno "Ready to load and modify a Rsyphon initrd image?" 0 0
